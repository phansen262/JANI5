...
public enum SnavLocations {
    ...
    //Returns Fragment from input string reference (can be used to determine location at runtime)
    public Fragment getFragment(Context c, String input){

        for(SnavLocations loc : SnavLocations.values()){
            if(c.getResources().getString(loc.mKey).equals(input)){
                return loc.mFrag;
            }
        }
